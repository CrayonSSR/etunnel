...
let fakeUserID = 'fake-id';
let fakeHostName = 'fake-host';
let noTLS = 'false';
...
function readURLConfig(reqArgs, handler) {
	let value;
...
function initSubConfig(env, url) {
	if (env.KEY) {
		有效时间 = env.TIME || 有效时间;
		更新时间 = env.UPTIME || 更新时间;
	}

	if (env.CFPORTS) { httpsPorts = formatEnvArg(env.CFPORTS) }
	readURLConfig([env, 'SUBNAME', url, '$sn'], v => FileName = v)
	readURLConfig([env, 'SUB', url, 'sub'], v => sub = v)
	...
	const AD_SPLIT_RULE = /[\r\n,，]+/i
	readURLConfig([env, 'ADD', url, '$ad'], v => addresses = v.split(AD_SPLIT_RULE))
	readURLConfig([env, 'ADDAPI', url, '$ada'], v => addressesapi = v.split(AD_SPLIT_RULE))
	if (env.ADDNOTLS) addressesnotls = formatEnvArg(env.ADDNOTLS);
	if (env.ADDNOTLSAPI) addressesnotlsapi = formatEnvArg(env.ADDNOTLSAPI);
	if (env.ADDCSV) addressescsv = formatEnvArg(env.ADDCSV);
...
export default {
	...
	async fetch(request, env, ctx) {
		try {
			...
			proxyIPs = formatEnvArg(env.PROXYIP || proxyIP);
			proxyIP = proxyIPs[0];
			if (env.$RD_PIP && proxyIPs.length > 1) {
				proxyIP = proxyIPs[Math.floor(Math.random() * proxyIPs.length)]
			}

			if (env.GO2SOCKS5) go2Socks5s = formatEnvArg(env.GO2SOCKS5);
			...
			if (!upgradeHeader || upgradeHeader !== 'websocket') {
				...
				switch (url.pathname.toLowerCase()) {
					case '/':
						...
						else { return new Response(JSON.stringify(request.cf, null, 4), { status: 200, headers: { 'content-type': 'application/json', }, }) }
					case `/${userID}`: {
...
async function proxyURL(proxyURL, url) {
	const URLs = formatEnvArg(proxyURL);
...
async function getAddressesapi(api) {
		...
		for (const [index, response] of responses.entries()) {
			...
			if (response.status === 'fulfilled') {
				...
				if (api[index].includes('proxyip=true')) {
					...
					proxyIPPool = proxyIPPool.concat((formatEnvArg(content)).map(item => {
	...
	const newAddressesapi = formatEnvArg(newapi);
